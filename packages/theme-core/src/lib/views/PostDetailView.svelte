<!--
  @view PostDetailView
  Controller: $pano/lib/pages/PostDetailPage.svelte
  Props:
    data.post           object — the post being displayed (title, text, category, writer, thumbnailUrl, views, date, url)
    data.previousPost   object|"-" — previous post ({ url, title }) or "-" when none
    data.nextPost       object|"-" — next post ({ url, title }) or "-" when none
    themeSettings       object — theme settings context (postCoverImageEnabled, postViewCountEnabled, postAuthorImageEnabled, postPreviousPageEnabled, postNextPageEnabled)
    avatarVersion       store — cache-busting value for profile picture URLs (use $avatarVersion)
  Override from a theme:
    theme.config.js → views: { PostDetailView: () => import("./src/views/PostDetailView.svelte") }
-->
<style lang="scss">
  .post-detail-cover {
    position: relative;
